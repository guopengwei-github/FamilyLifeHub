...
AGENT_API_KEY = "prometheus-agent-key-2026"

agent_api_key_header = APIKeyHeader(name="X-Agent-API-Key", auto_error=False)


async def verify_agent_api_key(api_key: str = Security(agent_api_key_header)) -> bool:
    """
    Verify Agent API key from request header.
    
    This is a specialized authentication for Prometheus Agent automation.
